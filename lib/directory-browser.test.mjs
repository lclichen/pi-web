...
test("lists directories and directory symlinks without returning files", async () => {
  ...
  try {
    ...
    // Directory symlinks need privilege on Windows; junctions don't and are
    // behaviorally identical for listDirectories (they satisfy isDirectory).
    const linkKind = process.platform === "win32" ? "junction" : undefined;
    await symlink(path.join(root, "project"), path.join(root, "linked-project"), linkKind);
