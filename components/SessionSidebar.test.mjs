...
test("manual and lifecycle refreshes bypass the server session-list cache", () => {
  // This fork routes the listing through the space-aware query builder instead
  // of upstream's inline ternary; the force flag still reaches /api/sessions.
  ...
  // The 0.9.0 merge replaced the manual refresh button with version-polled
  // refreshes; forced refresh survives via background-task completions.
  assert.match(source, /loadSessions\(false, true\);/);
});
