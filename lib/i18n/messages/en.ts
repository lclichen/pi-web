...
export const enLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "上传文件到此文件夹": "Upload files to this folder",
    ...
    "快速会话": "Quick",
    "快速": "Quick",
    "快速会话：无工作区的轻量对话": "Quick sessions: lightweight chats with no workspace",
    "暂无快速会话——点上方 ⚡ 按模板开始；每次选择模板都会新建一个独立会话。": "No quick sessions yet — pick a template via the ⚡ button above; every pick starts a NEW session.",
    "配置模板": "Config bundles",
    "启用": "Enable",
    "新建": "New",
    "内置": "Built-in",
    "暂无代理定义": "No agents defined",
    "保存中…": "Saving…",
    "保存": "Save",
    "收起": "Collapse",
    "工具": "Tools",
    "探测 MCP 工具": "Discover MCP tools",
    ...
    "处理中": "Working…",
    "已是最新版本。": "Up to date.",
    "当前部署形态不支持自助更新（开发环境或未知形态），请手动更新。": "Self-update is not available for this deployment (dev or unknown kind); update manually.",
    "需要管理员权限才能应用更新。": "Administrator rights are required to apply updates.",
    "更新完成。": "Update completed.",
    "更新失败。": "Update failed.",
    "服务重启中，页面稍后自动恢复…": "Services restarting; this page will reconnect shortly…",
    "已提交…": "Submitted…",
    "等待服务恢复超时——请手动检查服务状态。": "Timed out waiting for services to come back — check service status manually.",
    "暂无 MCP 服务器": "No MCP servers configured",
    "刷新工具": "Refresh tools",
    "收起设置": "Hide settings",
    "设置": "Settings",
    "保存设置": "Save settings",
    ...
    "探测": "Probe",
    "探测中…": "Probing…",
    "选择或新建 MCP 服务器": "Select a server or create a new one",
    "展开": "Expand",
    "内置代理为只读；可创建同名 project 代理来覆盖。": "Built-in agents are read-only; create a same-named project agent to override.",
    "新增模板": "New template",
    "模板已创建。": "Template created.",
    "模板已保存。": "Template saved.",
    "暂无模板": "No templates yet",
    "加载模板…": "Loading templates…",
    "加载配置模板…": "Loading config bundles…",
    "模板名称（用户可见，如「产品A助手」）": "Template name (user-visible, e.g. “Product A”)",
    "描述（可选）": "Description (optional)",
    "系统提示词": "System prompt",
    "模型 Provider（可选，如 modelscope）": "Model provider (optional, e.g. modelscope)",
    "模型 ID（可选）": "Model ID (optional)",
    "默认用用户当前模型": "Defaults to the user's current model",
    "MCP 服务器允许列表（可选，逗号分隔服务器名；留空 = 用户配置的全部服务器）": "MCP server allowlist (optional, comma-separated; empty = all user-configured servers)",
    "默认": "Default",
    "默认 · 不可删除": "Default · not deletable",
    "上传模板": "Upload bundle",
    "上传": "Upload",
    "上传中…": "Uploading…",
    "模板已上传。": "Bundle uploaded.",
    "上传时间": "Uploaded",
    "选择 zip / apkg 文件": "Choose a zip / apkg file",
    "请选择一个 zip 或 apkg 文件": "Please choose a zip or apkg file",
    "模板名（可选，默认用文件名；字母数字点下划线连字符）": "Bundle name (optional, defaults to file name; letters, digits, dot, underscore, hyphen)",
    "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip 或加密 .apkg）。用户新建项目时可在向导中选择；已有项目可通过项目菜单「套用配置模板…」补装。": "Config bundles are standard .pi/ config + labs/ archives (zip, or encrypted .apkg) maintained by admins. Pick one in the new-project wizard; existing projects can install via the project menu “Apply config bundle…”.",
    "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip 或加密 .apkg）。选择左侧模板查看详情，或点击左下角「上传模板」。": "Config bundles are standard .pi/ config + labs/ archives (zip, or encrypted .apkg) maintained by admins. Select a bundle on the left for details, or click “Upload bundle” at the bottom left.",
    "加密": "Encrypted",
    "加密配置包（apkg）": "Encrypted package (apkg)",
    "明文配置包（zip）": "Plain package (zip)",
    "类型": "Type",
    "说明": "Notes",
    "套用时内存解密；套用后该项目禁止再导出配置（防二次分发）。": "Decrypted in memory on apply; the project can no longer export its config afterwards (anti-redistribution).",
    "加密配置包：套用时内存解密；套用后项目禁止再导出配置。": "Encrypted package: decrypted in memory on apply; re-export is blocked after applying.",
    "模板决定快速会话的系统提示词、模型与 MCP 白名单；default 可编辑不可删除。选择左侧模板，或点击左下角「新增模板」。": "Templates drive a quick session’s system prompt, model and MCP allowlist; default is editable but not deletable. Select a template on the left, or click “New template” at the bottom left.",
    "确定删除模板「{name}」吗？": "Delete template “{name}”?",
    "确定删除模板「{name}」吗？已套用它的项目不受影响。": "Delete bundle “{name}”? Projects already using it are unaffected.",
    "模板决定快速会话的系统提示词、模型与 MCP 白名单；default 可编辑不可删除。": "Templates drive a quick session’s system prompt, model and MCP allowlist; default is editable but not deletable.",
    "不使用模板": "No template",
    "测试中…": "Testing…",
    "测试连接": "Test connection",
    "连接成功：{user}": "Connected: {user}",
    "请先填写项目名称": "Enter a project name first",
    "平台管理（用户/密码重置；完整控制台在面板内）": "Platform admin (users & password reset; full console inside the panel)",
    "用户（P1）· 容器/镜像等后续批次迁入": "Users (P1) — containers/images arrive in later batches",
    "完整控制台 ↗": "Full console ↗",
    "完整管理台（旧版控制台，功能逐步迁入）": "Full admin console (legacy; features migrate in gradually)",
    "搜索用户名/邮箱": "Search username / email",
    "全部状态": "All statuses",
    "+ 新建用户": "+ New user",
    "已重置 {name} 的密码，其所有旧会话已失效": "Password reset for {name}; all their old sessions are now invalid",
    "已将 {name} 置为 {status}": "{name} set to {status}",
    "确定删除用户 {name}？其容器与工作区将一并清理": "Delete user {name}? Their containers and workspaces will be cleaned up too",
    "已删除 {name}": "Deleted {name}",
    "已批准 {name}": "Approved {name}",
    "已拒绝 {name}": "Rejected {name}",
    "没有匹配的用户": "No matching users",
    "下次登录须改密": "Must change password at next login",
    "批准": "Approve",
    "拒绝": "Reject",
    "重置密码": "Reset password",
    "停用": "Disable",
    "共 {total} 人": "{total} users",
    "上一页": "Prev",
    "下一页": "Next",
    "重置 {name} 的密码": "Reset password for {name}",
    "重置后该用户的所有会话与登录态立即失效；请将新密码安全地告知用户（至少 8 字符）。": "All of this user's sessions and logins become invalid immediately; hand over the new password securely (min 8 characters).",
    "新密码（≥8 字符）": "New password (≥8 chars)",
    "确定重置": "Reset",
    "角色": "Role",
    "创建失败（用户名重复或密码不符合策略）": "Creation failed (duplicate username or password policy violation)",
    "初始密码（≥8 字符）": "Initial password (≥8 chars)",
    "已创建用户 {name}": "Created user {name}",
    "搜索": "Search",
    "加载中…": "Loading…",
    "操作": "Actions",
    "用户": "Users",
    "容器": "Containers",
    "镜像": "Images",
    "最多显示最近 50 个": "Showing the 50 most recent",
    "没有容器": "No containers",
    "已启动容器 {name}": "Started container {name}",
    "已停止容器 {name}": "Stopped container {name}",
    "确定删除容器 {name}（{owner}）？实例与可写层将被清理": "Delete container {name} ({owner})? The instance and writable layer will be cleaned up",
    "已删除容器 {name}": "Deleted container {name}",
    "名称": "Name",
    "所有者": "Owner",
    "镜像目录（sif 需先放到平台可访问的路径）": "Image catalogue (place the .sif on a platform-accessible path first)",
    "已注册镜像 {name}": "Registered image {name}",
    "显示名": "Display name",
    "可见性": "Visibility",
    "镜像目录为空": "Image catalogue is empty",
    "公开": "Public",
    "私有": "Private",
    "镜像 {name} 已设为公开": "Image {name} is now public",
    "镜像 {name} 已设为私有": "Image {name} is now private",
    "确定删除镜像 {name}？（不影响已用该镜像创建的容器）": "Delete image {name}? (existing containers built from it are unaffected)",
    "已删除镜像 {name}": "Deleted image {name}",
    "+ 注册镜像": "+ Register image",
    "名称（字母/数字/_./-）": "Name (letters/digits/_./-)",
    "注册": "Register",
    "资源": "Resources",
    "停止": "Stop",
    "启动": "Start",
    ...
    "镜像数": "Images",
    "24h 失败操作": "Failed ops (24h)",
    "执行器：{kind} · 数据库：{dialect}": "Executor: {kind} · Database: {dialect}",
    "配额层级（限制每用户可创建的容器/工作区数量与资源）": "Quota tiers (per-user caps on containers/workspaces and resources)",
    "+ 新建配额": "+ New quota",
    "确定删除配额 {name}？使用该配额的用户将失去配额限制依据": "Delete quota {name}? Users on it lose their quota basis",
    "已删除配额 {name}": "Deleted quota {name}",
    "配额已创建": "Quota created",
    "配额已更新": "Quota updated",
    "快照/容器": "Snaps/container",
    "编辑": "Edit",
    "描述": "Description",
    "容器上限": "Max containers",
    "内存 (GB)": "Memory (GB)",
    ...
    "内存": "Memory",
    "磁盘": "Disk",
    "搜索工作区名称": "Search workspace name",
    "确定删除工作区 {name}（{owner}）？存储目录将一并删除": "Delete workspace {name} ({owner})? Its storage directory is removed too",
    "已删除工作区 {name}": "Deleted workspace {name}",
    "没有工作区": "No workspaces",
    "大小": "Size",
    "文件数": "Files",
    "来源容器": "From container",
    "个": "items",
    "LLM 访问授权（绑定平台用户与虚拟密钥预算）": "LLM access grants (platform users bound to virtual-key budgets)",
    "用户 #id": "User #id",
    "预算": "Budget",
    "授权": "Grant",
    "已授权用户 #{id}（预算 {budget}/月）": "Granted user #{id} (budget {budget}/mo)",
    "确定撤销用户 #{id} 的 LLM 访问？其虚拟密钥将立即失效": "Revoke LLM access for user #{id}? Their virtual keys die immediately",
    "已撤销用户 #{id} 的 LLM 访问": "Revoked LLM access for user #{id}",
    "没有 LLM 授权": "No LLM grants",
    "周期": "Cycle",
    "模型": "Models",
    "全部": "All",
    "已撤销": "Revoked",
    "生效中": "Active",
    "撤销": "Revoke",
    "按操作过滤（如 container.create）": "Filter by action (e.g. container.create)",
    "没有日志": "No logs",
    "时间": "Time",
    "条": "entries",
  },
