...
test("rejects files outside cwd, including symlink targets", async (t) => {
  ...
  try {
    ...
    // Mode / admin). Without it the fixture cannot exist — the direct-path
    // rejection above still ran, so only skip the symlink-escape branch.
    try {
      await symlink(outsideFile, join(cwd, "linked.md"));
    } catch (e) {
      if (e?.code === "EPERM" && process.platform === "win32") {
        return t.skip("symlink privilege unavailable (Windows without Developer Mode)");
      }
      throw e;
    }
