...
test("forced session listing bypasses the fresh server cache", { skip: "upstream incremental-scanner seam (listSessions) not adopted in this fork" }, async (t) => {
  const originalListSessions = listSessions;
  let scans = 0;
  listSessions = async () => {
    scans += 1;
...
test("a scan invalidated in flight retries before returning to its caller", { skip: "upstream incremental-scanner seam (listSessions) not adopted in this fork" }, async (t) => {
  const originalListSessions = listSessions;
  let scans = 0;
