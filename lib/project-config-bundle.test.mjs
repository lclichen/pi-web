...
function makeHome() {
  ...
  // Session plans are runtime state, not portable config.
  mkdirSync(join(home, ".pi", "plans"), { recursive: true });
  writeFileSync(join(home, ".pi", "plans", "plan-sess_demo.md"), "# plan");
...
test("export strips credentials and machine state, keeps config and labs", async () => {
  ...
  try {
    ...
    assert.ok(!names.some((n) => n.includes("plans")), "session plans are runtime state, not config");
