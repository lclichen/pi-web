...
// POST /api/admin/users/[id]/reject — approval-queue action for a pending
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const user = await platformPost<unknown>("/api/v1/admin/users/" + id + "/reject", identity.session.apiKey);
