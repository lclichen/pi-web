...
## Coverage Status & Backlog (2026-08)

Audit tool: `node scripts/audit-i18n-cjk.cjs` — lists **bare CJK** lines that
bypass `t()`/`translate()` (i18n-wrapped lines and comments are filtered out).

- The existing 378 `t()`/`translate()` keys have **zero missing entries** in
  both `en.ts` and `zh-CN.ts`.
- Newly converted to `t()` this round: `AppShell` top bar, `ChatStatusWidget`,
  `NewProjectDialog`, `ProjectSessionTree`, `FileExplorer` new-file buttons
  (~110 strings). These use the **Chinese-as-key** pattern: the code says
  `t("登出")` and only `en.ts` needs a `"登出": "Log out"` mapping
  (`zh-CN.ts` carries self-mapping entries so dev warnings stay quiet).

### Backlog (bare-CJK surfaces, highest visibility first)

| File | Bare lines | Notes |
| --- | --- | --- |
| components/SandboxManagerDialog.tsx | 38 | container management modal |
| components/relay/ConnectLocalMachine.tsx | 30 | local-machine pairing |
| components/relay/LocalMachinePanel.tsx | 24 | local-machine panel |
| components/SubagentDirectoryPanel.tsx | 19 | subagent directory |
| app/login/page.tsx | 18 | login/register form |
| components/WorkspaceTerminal.tsx | 16 | terminal banners (ANSI-aware) |
| components/MyWorkspaceDialog.tsx | 14 | cloud workspace modal |
| components/PluginsConfig.tsx | 14 | plugins page |
| components/ProjectSettingsDialog.tsx | 14 | project settings modal |
| components/ProjectImportDialog.tsx | 11 | import modal |
| app/api/** error messages | ~60 | server messages; Accept-Language negotiation is a larger change and stays Chinese for now |

When adding new UI, prefer the Chinese-as-key pattern shown above and add the
English mapping to `lib/i18n/messages/en.ts`.
