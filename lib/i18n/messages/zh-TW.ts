...
export const zhTWLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "正在测试 SSH 连接…": "正在測試 SSH 連線…",
    "重试": "重試",
    "请返回上一步检查主机地址、端口与认证信息，或重试。": "請返回上一步檢查主機位址、連接埠與認證資訊，或重試。",
    "远程工作目录（可选）": "远程工作目录（可选）",
    "选择或填写远程主机上该项目的工作目录（例如 /root/projects/demo；留空则使用远程 home）。": "選擇或填寫遠端主機上該項目的工作目錄（例如 /root/projects/demo；留空則使用遠端 home）。",
    "浏览远程目录": "瀏覽遠端目錄",
    "打开远程目录浏览器": "打開遠端目錄瀏覽器",
    "请选择 / 以外的目录": "請選擇 / 以外的目錄",
    ...
    "快速会话": "快速會話",
    "快速": "快速",
    "快速会话：无工作区的轻量对话": "快速會話：無工作區的輕量對話",
    "暂无快速会话——点上方 ⚡ 按模板开始；每次选择模板都会新建一个独立会话。": "暫無快速會話——點上方 ⚡ 按模板開始；每次選擇模板都會新建一個獨立會話。",
    "配置模板": "配置模板",
    "启用": "啟用",
    "新建": "新建",
    "内置": "內置",
    ...
    "处理中": "處理中",
    "已是最新版本。": "已是最新版本。",
    "当前部署形态不支持自助更新（开发环境或未知形态），请手动更新。": "目前部署形態不支援自助更新（開發環境或未知形態），請手動更新。",
    "需要管理员权限才能应用更新。": "需要管理員權限才能應用更新。",
    "更新完成。": "更新完成。",
    "更新失败。": "更新失敗。",
    "服务重启中，页面稍后自动恢复…": "服務重啟中，頁面稍後自動恢復…",
    "已提交…": "已提交…",
    "等待服务恢复超时——请手动检查服务状态。": "等待服務恢復超時——請手動檢查服務狀態。",
    "暂无 MCP 服务器": "暫無 MCP 伺服器",
    "刷新工具": "重新整理工具",
    "收起设置": "收起設定",
    "设置": "設定",
    "保存设置": "儲存設定",
    ...
    "探测": "探測",
    "探测中…": "探測中…",
    "选择或新建 MCP 服务器": "選擇或新建 MCP 伺服器",
    "展开": "展開",
    "内置代理为只读；可创建同名 project 代理来覆盖。": "內置代理為唯讀；可建立同名 project 代理來覆蓋。",
    "新增模板": "新增模板",
    "暂无模板": "暫無模板",
    "加载模板…": "載入模板…",
    "加载配置模板…": "載入配置模板…",
    ...
    "上传": "上傳",
    "上传中…": "上傳中…",
    "选择 zip / apkg 文件": "選擇 zip / apkg 檔案",
    "请选择一个 zip 或 apkg 文件": "請選擇一個 zip 或 apkg 檔案",
    "模板名称（用户可见，如「产品A助手」）": "模板名稱（使用者可見，如「產品A助手」）",
    "描述（可选）": "描述（可選）",
    "系统提示词": "系統提示詞",
    "模型 Provider（可选，如 modelscope）": "模型 Provider（可選，如 modelscope）",
    "模型 ID（可选）": "模型 ID（可選）",
    "默认用用户当前模型": "預設用使用者目前模型",
    "MCP 服务器允许列表（可选，逗号分隔服务器名；留空 = 用户配置的全部服务器）": "MCP 伺服器允許清單（可選，逗號分隔伺服器名稱；留空 = 使用者設定的全部伺服器）",
    "默认": "預設",
    "默认 · 不可删除": "預設 · 不可刪除",
    "模板名（可选，默认用文件名；字母数字点下划线连字符）": "模板名（可選，預設用檔案名稱；字母數字點底線連字號）",
    "确定删除模板「{name}」吗？": "確定刪除模板「{name}」嗎？",
    "确定删除模板「{name}」吗？已套用它的项目不受影响。": "確定刪除模板「{name}」嗎？已套用它的專案不受影響。",
    "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip 或加密 .apkg）。用户新建项目时可在向导中选择；已有项目可通过项目菜单「套用配置模板…」补装。": "配置模板是管理員維護的標準 .pi/ 配置 + labs/ 打包（zip 或加密 .apkg）。使用者新建專案時可在嚮導中選擇；已有專案可透過專案選單「套用配置模板…」補裝。",
    "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip 或加密 .apkg）。选择左侧模板查看详情，或点击左下角「上传模板」。": "配置模板是管理員維護的標準 .pi/ 配置 + labs/ 打包（zip 或加密 .apkg）。選擇左側模板查看詳情，或點擊左下角「上傳模板」。",
    "加密": "加密",
    "加密配置包（apkg）": "加密配置包（apkg）",
    "明文配置包（zip）": "明文配置包（zip）",
    "类型": "類型",
    "说明": "說明",
    "套用时内存解密；套用后该项目禁止再导出配置（防二次分发）。": "套用時記憶體解密；套用後該專案禁止再匯出配置（防二次散布）。",
    "加密配置包：套用时内存解密；套用后项目禁止再导出配置。": "加密配置包：套用時記憶體解密；套用後專案禁止再匯出配置。",
    "模板决定快速会话的系统提示词、模型与 MCP 白名单；default 可编辑不可删除。选择左侧模板，或点击左下角「新增模板」。": "模板決定快速會話的系統提示詞、模型與 MCP 白名單；default 可編輯不可刪除。選擇左側模板，或點擊左下角「新增模板」。",
    "模板决定快速会话的系统提示词、模型与 MCP 白名单；default 可编辑不可删除。": "模板決定快速會話的系統提示詞、模型與 MCP 白名單；default 可編輯不可刪除。",
    "不使用模板": "不使用模板",
    "测试中…": "测试中…",
    "测试连接": "测试连接",
    "连接成功：{user}": "连接成功：{user}",
    "请先填写项目名称": "请先填写项目名称",
    "平台管理（用户/密码重置；完整控制台在面板内）": "平台管理（使用者/密碼重置；完整控制台在面板內）",
    "用户（P1）· 容器/镜像等后续批次迁入": "使用者（P1）· 容器/鏡像等後續批次遷入",
    "完整控制台 ↗": "完整控制台 ↗",
    "完整管理台（旧版控制台，功能逐步迁入）": "完整管理台（舊版控制台，功能逐步遷入）",
    "搜索用户名/邮箱": "搜尋使用者名稱/信箱",
    "全部状态": "全部狀態",
    "搜索": "搜尋",
    "+ 新建用户": "+ 新增使用者",
    "已重置 {name} 的密码，其所有旧会话已失效": "已重置 {name} 的密碼，其所有舊工作階段已失效",
    "已将 {name} 置为 {status}": "已將 {name} 置為 {status}",
    "确定删除用户 {name}？其容器与工作区将一并清理": "確定刪除使用者 {name}？其容器與工作區將一併清理",
    "已删除 {name}": "已刪除 {name}",
    "已批准 {name}": "已批准 {name}",
    "已拒绝 {name}": "已拒絕 {name}",
    "加载中…": "載入中…",
    ...
    "上一页": "上一頁",
    "下一页": "下一頁",
    "重置 {name} 的密码": "重置 {name} 的密碼",
    "重置后该用户的所有会话与登录态立即失效；请将新密码安全地告知用户（至少 8 字符）。": "重置後該使用者的所有工作階段與登入狀態立即失效；請將新密碼安全地告知使用者（至少 8 字元）。",
    "新密码（≥8 字符）": "新密碼（≥8 字元）",
    "确定重置": "確定重置",
    "角色": "角色",
    "操作": "操作",
    "创建失败（用户名重复或密码不符合策略）": "建立失敗（使用者名稱重複或密碼不符合策略）",
    "初始密码（≥8 字符）": "初始密碼（≥8 字元）",
    "已创建用户 {name}": "已建立使用者 {name}",
    "用户": "使用者",
    "容器": "容器",
    "镜像": "鏡像",
    "最多显示最近 50 个": "最多顯示最近 50 個",
    "没有容器": "沒有容器",
    "已启动容器 {name}": "已啟動容器 {name}",
    "已停止容器 {name}": "已停止容器 {name}",
    "确定删除容器 {name}（{owner}）？实例与可写层将被清理": "確定刪除容器 {name}（{owner}）？實例與可寫層將被清理",
    "已删除容器 {name}": "已刪除容器 {name}",
    "名称": "名稱",
    "所有者": "擁有者",
    "镜像目录（sif 需先放到平台可访问的路径）": "鏡像目錄（sif 需先放到平台可存取的路徑）",
    "已注册镜像 {name}": "已註冊鏡像 {name}",
    "显示名": "顯示名",
    "可见性": "可見性",
    "镜像目录为空": "鏡像目錄為空",
    "公开": "公開",
    "私有": "私有",
    "镜像 {name} 已设为公开": "鏡像 {name} 已設為公開",
    "镜像 {name} 已设为私有": "鏡像 {name} 已設為私有",
    "确定删除镜像 {name}？（不影响已用该镜像创建的容器）": "確定刪除鏡像 {name}？（不影響已用該鏡像建立的容器）",
    "已删除镜像 {name}": "已刪除鏡像 {name}",
    "+ 注册镜像": "+ 註冊鏡像",
    "名称（字母/数字/_./-）": "名稱（字母/數字/_./-）",
    "注册": "註冊",
    "资源": "資源",
    "停止": "停止",
    "启动": "啟動",
    ...
    "镜像数": "鏡像數",
    "24h 失败操作": "24h 失敗操作",
    "执行器：{kind} · 数据库：{dialect}": "執行器：{kind} · 資料庫：{dialect}",
    "配额层级（限制每用户可创建的容器/工作区数量与资源）": "配額層級（限制每使用者可建立的容器/工作區數量與資源）",
    "+ 新建配额": "+ 新增配額",
    "确定删除配额 {name}？使用该配额的用户将失去配额限制依据": "確定刪除配額 {name}？使用該配額的使用者將失去配額限制依據",
    "已删除配额 {name}": "已刪除配額 {name}",
    "配额已创建": "配額已建立",
    "配额已更新": "配額已更新",
    "快照/容器": "快照/容器",
    "编辑": "編輯",
    ...
    "磁盘": "磁碟",
    "搜索工作区名称": "搜尋工作區名稱",
    "确定删除工作区 {name}（{owner}）？存储目录将一并删除": "確定刪除工作區 {name}（{owner}）？儲存目錄將一併刪除",
    "已删除工作区 {name}": "已刪除工作區 {name}",
    "没有工作区": "沒有工作區",
    "大小": "大小",
    "文件数": "檔案數",
    "来源容器": "來源容器",
    "个": "個",
    "LLM 访问授权（绑定平台用户与虚拟密钥预算）": "LLM 存取授權（綁定平台使用者與虛擬金鑰預算）",
    "用户 #id": "使用者 #id",
    "预算": "預算",
    "授权": "授權",
    "已授权用户 #{id}（预算 {budget}/月）": "已授權使用者 #{id}（預算 {budget}/月）",
    "确定撤销用户 #{id} 的 LLM 访问？其虚拟密钥将立即失效": "確定撤銷使用者 #{id} 的 LLM 存取？其虛擬金鑰將立即失效",
    "已撤销用户 #{id} 的 LLM 访问": "已撤銷使用者 #{id} 的 LLM 存取",
    "没有 LLM 授权": "沒有 LLM 授權",
    "周期": "週期",
    "模型": "模型",
    "全部": "全部",
    ...
    "生效中": "生效中",
    "撤销": "撤銷",
    "按操作过滤（如 container.create）": "按操作過濾（如 container.create）",
    "没有日志": "沒有日誌",
    "时间": "時間",
    "条": "條",
  },
