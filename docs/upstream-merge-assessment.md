...
> 已 cherry-pick **35 个**高价值提交（四批）；另经核对，**v0.9.0 squash 合并（`a249552`）已带入 51 个提交的内容**（见文末 09-22 更新）。
...
1. **中等档逐个**：`3f07a5f`（推理级别显示）价值最高，建议单独会话
2. **下次合并窗口**：`ed50d88`（侧栏拖拽调宽）、`448e146`（主题系统，最大）等大 UI 功能
3. **等上游 v0.10**：考虑整体 merge（到时冲突面可能更大但一次性解决）
4. **持续跳过**：`237d0ca`（内置子智能体）、`e5a2434`（Next.js 16 独立评估）、上游密码登录体系
...
> **2026-09-22 第四批已带走 `50a2fd4`；剩约 13 个。**

`3f07a5f` 运行中 turn 的推理级别（13 文件 8 分歧；目标模型为推理型，价值高）、
`1eb5e66` 滚到底部按钮（9 文件 6 分歧，含 chat-lazy-load 新模块）、
`def1478` SSE 重连保留 shell 输出（rpc-manager 重分歧）、`4787a14` 打开活跃会话保留流式输出、
`0e71201` 大文本预览分页（8 文件 6 分歧）、`ed840ed`+`b1a7296` 会话模型准确恢复（9 文件 8 分歧）、
`d10988d` 重复内建命令提交、`dab9850`+`1b88ec7` @ 选择器键盘/定位、`ef51ffd` 插件设置显示顶层扩展、
`585d56c` 首消息会话 fork、`b4a4539` PWA fetch 上限防挂死（fork 有 sw.js）。

### 大功能：专用合并窗口（约 8 个）

`448e146` 可读性主题 + 主题选择器（21 文件 14 分歧，最大）、`ffb2daf` 文件面板全宽开关（8 文件）、
`ed50d88` 侧栏拖拽调宽、`f2d600b` /auto-compact、`5e9b997` PDF #page= 跳转（13 文件多为测试）、
`6d53fd5`+`6e95fba` provider 用量配额（目标模型本地/开源，价值存疑）、
`3e9fcfa` iOS 推送后台送达（fork 有 web-push；教学场景 iOS 优先级低）、`2eb95b9` 设置页主题/语言控件（与 448e146 绑定）。
