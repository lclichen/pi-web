...
export const zhTWLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip）。用户新建项目时可在向导中选择；已有项目可通过项目菜单「套用配置模板…」补装。": "配置模板是管理員維護的標準 .pi/ 配置 + labs/ 打包（zip）。使用者新建專案時可在嚮導中選擇；已有專案可透過專案選單「套用配置模板…」補裝。",
    "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip）。选择左侧模板查看详情，或点击左下角「上传模板」。": "配置模板是管理員維護的標準 .pi/ 配置 + labs/ 打包（zip）。選擇左側模板查看詳情，或點擊左下角「上傳模板」。",
