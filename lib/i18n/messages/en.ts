...
export const enLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "请选择一个 zip 文件": "Please choose a zip file",
    "模板名（可选，默认用文件名；字母数字点下划线连字符）": "Bundle name (optional, defaults to file name; letters, digits, dot, underscore, hyphen)",
    "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip）。用户新建项目时可在向导中选择；已有项目可通过项目菜单「套用配置模板…」补装。": "Config bundles are standard .pi/ config + labs/ archives (zip) maintained by admins. Pick one in the new-project wizard; existing projects can install via the project menu “Apply config bundle…”.",
    "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip）。选择左侧模板查看详情，或点击左下角「上传模板」。": "Config bundles are standard .pi/ config + labs/ archives (zip) maintained by admins. Select a bundle on the left for details, or click “Upload bundle” at the bottom left.",
