...
test("package extension is never stripped (2026-09 merge decision)", () => {
  const host = {
    path: "<inline:pi-web-subagents>",
    tools: new Map([["Agent", {}], ["get_subagent_result", {}], ["steer_subagent", {}]]),
  };
  const pkg = {
    path: "/home/u/.pi/agent/npm/node_modules/@tintinweb/pi-subagents/src/index.ts",
    tools: new Map([["Agent", {}], ["get_subagent_result", {}], ["steer_subagent", {}]]),
  };
  const base = {
    extensions: [pkg, host],
    errors: [{ path: host.path, error: `Tool "Agent" conflicts with ${pkg.path}` }],
  };
  const result = preferPiWebSubagentExtension(base);
