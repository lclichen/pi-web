...
function resetSessionListState() {
  globalThis.__piSessionSpaceCaches = undefined;
...
test("session listing reads subagent relations and terminal status without reopening full session files", async (t) => {
  ...
  const sessions = await listAllSessions({ kind: "host" }, { force: true });
...
test("keeps forward and reverse session path caches in sync", async () => {
  ...
  assert.equal(spaceBundle("host").pathCache.has(sessionId), false);
  assert.equal(spaceBundle("host").idToPathCache.has(sessionPathKey(filePath)), false);
});
...
test("resolves a matching session header without a catalogue scan", async (t) => {
  ...
  assert.equal(spaceBundle("host").idToPathCache.get(sessionPathKey(filePath)), sessionId);

  resetSessionPathState();
  assert.equal(await resolveSessionIdByPath(filePath), sessionId);
  assert.equal(scans, 0);
  assert.equal(spaceBundle("host").pathCache.get(sessionId), filePath);
});

test("does not resolve a parent path outside the default session storage", { skip: "upstream incremental-scanner seam (listSessions) not adopted in this fork" }, async (t) => {
...
test("falls back to the catalogue when a targeted candidate header is invalid", { skip: "upstream incremental-scanner seam (listSessions) not adopted in this fork" }, async (t) => {
...
test("falls back to the catalogue when a targeted lookup finds duplicate IDs", { skip: "upstream incremental-scanner seam (listSessions) not adopted in this fork" }, async (t) => {
...
test("forced session listing bypasses the fresh server cache", { skip: "requires upstream incremental scanner (not adopted: per-space cache in this fork)" }, async (t) => {
  const originalListSessions = listSessions;
  let scans = 0;
  listSessions = async () => {
    scans += 1;
...
test("a scan invalidated in flight retries before returning to its caller", { skip: "requires upstream incremental scanner (not adopted: per-space cache in this fork)" }, async (t) => {
