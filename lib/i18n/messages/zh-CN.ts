...
export const zhCNLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "快速会话": "快速会话",
    "快速": "快速",
    "快速会话：无工作区的轻量对话": "快速会话：无工作区的轻量对话",
    "暂无快速会话——点上方 ⚡ 按模板开始；每次选择模板都会新建一个独立会话。": "暂无快速会话——点上方 ⚡ 按模板开始；每次选择模板都会新建一个独立会话。",
    "配置模板": "配置模板",
    "启用": "启用",
    "新建": "新建",
    "内置": "内置",
    ...
    "处理中": "处理中",
    "已是最新版本。": "已是最新版本。",
    "当前部署形态不支持自助更新（开发环境或未知形态），请手动更新。": "当前部署形态不支持自助更新（开发环境或未知形态），请手动更新。",
    "需要管理员权限才能应用更新。": "需要管理员权限才能应用更新。",
    "更新完成。": "更新完成。",
    "更新失败。": "更新失败。",
    "服务重启中，页面稍后自动恢复…": "服务重启中，页面稍后自动恢复…",
    "已提交…": "已提交…",
    "等待服务恢复超时——请手动检查服务状态。": "等待服务恢复超时——请手动检查服务状态。",
    ...
    "探测": "探测",
    "探测中…": "探测中…",
    "选择或新建 MCP 服务器": "选择或新建 MCP 服务器",
    "展开": "展开",
    "内置代理为只读；可创建同名 project 代理来覆盖。": "内置代理为只读；可创建同名 project 代理来覆盖。",
    "新增模板": "新增模板",
    "暂无模板": "暂无模板",
    "加载模板…": "加载模板…",
    "加载配置模板…": "加载配置模板…",
    ...
    "上传": "上传",
    "上传中…": "上传中…",
    "选择 zip / apkg 文件": "选择 zip / apkg 文件",
    "请选择一个 zip 或 apkg 文件": "请选择一个 zip 或 apkg 文件",
    "模板名称（用户可见，如「产品A助手」）": "模板名称（用户可见，如「产品A助手」）",
    "描述（可选）": "描述（可选）",
    "系统提示词": "系统提示词",
    "模型 Provider（可选，如 modelscope）": "模型 Provider（可选，如 modelscope）",
    "模型 ID（可选）": "模型 ID（可选）",
    "默认用用户当前模型": "默认用用户当前模型",
    "MCP 服务器允许列表（可选，逗号分隔服务器名；留空 = 用户配置的全部服务器）": "MCP 服务器允许列表（可选，逗号分隔服务器名；留空 = 用户配置的全部服务器）",
    "默认": "默认",
    "默认 · 不可删除": "默认 · 不可删除",
    "模板名（可选，默认用文件名；字母数字点下划线连字符）": "模板名（可选，默认用文件名；字母数字点下划线连字符）",
    "确定删除模板「{name}」吗？": "确定删除模板「{name}」吗？",
    "确定删除模板「{name}」吗？已套用它的项目不受影响。": "确定删除模板「{name}」吗？已套用它的项目不受影响。",
    "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip 或加密 .apkg）。用户新建项目时可在向导中选择；已有项目可通过项目菜单「套用配置模板…」补装。": "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip 或加密 .apkg）。用户新建项目时可在向导中选择；已有项目可通过项目菜单「套用配置模板…」补装。",
    "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip 或加密 .apkg）。选择左侧模板查看详情，或点击左下角「上传模板」。": "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip 或加密 .apkg）。选择左侧模板查看详情，或点击左下角「上传模板」。",
    "加密": "加密",
    "加密配置包（apkg）": "加密配置包（apkg）",
    "明文配置包（zip）": "明文配置包（zip）",
    "类型": "类型",
    "说明": "说明",
    "套用时内存解密；套用后该项目禁止再导出配置（防二次分发）。": "套用时内存解密；套用后该项目禁止再导出配置（防二次分发）。",
    "加密配置包：套用时内存解密；套用后项目禁止再导出配置。": "加密配置包：套用时内存解密；套用后项目禁止再导出配置。",
    "模板决定快速会话的系统提示词、模型与 MCP 白名单；default 可编辑不可删除。选择左侧模板，或点击左下角「新增模板」。": "模板决定快速会话的系统提示词、模型与 MCP 白名单；default 可编辑不可删除。选择左侧模板，或点击左下角「新增模板」。",
    "模板决定快速会话的系统提示词、模型与 MCP 白名单；default 可编辑不可删除。": "模板决定快速会话的系统提示词、模型与 MCP 白名单；default 可编辑不可删除。",
    "不使用模板": "不使用模板",
    "测试中…": "测试中…",
    "测试连接": "测试连接",
    "连接成功：{user}": "连接成功：{user}",
    "请先填写项目名称": "请先填写项目名称",
    "平台管理（用户/密码重置；完整控制台在面板内）": "平台管理（用户/密码重置；完整控制台在面板内）",
    "用户（P1）· 容器/镜像等后续批次迁入": "用户（P1）· 容器/镜像等后续批次迁入",
    "完整控制台 ↗": "完整控制台 ↗",
    "完整管理台（旧版控制台，功能逐步迁入）": "完整管理台（旧版控制台，功能逐步迁入）",
    "搜索用户名/邮箱": "搜索用户名/邮箱",
    "全部状态": "全部状态",
    "搜索": "搜索",
    "+ 新建用户": "+ 新建用户",
    "已重置 {name} 的密码，其所有旧会话已失效": "已重置 {name} 的密码，其所有旧会话已失效",
    "已将 {name} 置为 {status}": "已将 {name} 置为 {status}",
    "确定删除用户 {name}？其容器与工作区将一并清理": "确定删除用户 {name}？其容器与工作区将一并清理",
    "已删除 {name}": "已删除 {name}",
    "已批准 {name}": "已批准 {name}",
    "已拒绝 {name}": "已拒绝 {name}",
    "加载中…": "加载中…",
    ...
    "上一页": "上一页",
    "下一页": "下一页",
    "重置 {name} 的密码": "重置 {name} 的密码",
    "重置后该用户的所有会话与登录态立即失效；请将新密码安全地告知用户（至少 8 字符）。": "重置后该用户的所有会话与登录态立即失效；请将新密码安全地告知用户（至少 8 字符）。",
    "新密码（≥8 字符）": "新密码（≥8 字符）",
    "确定重置": "确定重置",
    "角色": "角色",
    "操作": "操作",
    "创建失败（用户名重复或密码不符合策略）": "创建失败（用户名重复或密码不符合策略）",
    "初始密码（≥8 字符）": "初始密码（≥8 字符）",
    "已创建用户 {name}": "已创建用户 {name}",
    "用户": "用户",
    "容器": "容器",
    "镜像": "镜像",
    "最多显示最近 50 个": "最多显示最近 50 个",
    "没有容器": "没有容器",
    "已启动容器 {name}": "已启动容器 {name}",
    "已停止容器 {name}": "已停止容器 {name}",
    "确定删除容器 {name}（{owner}）？实例与可写层将被清理": "确定删除容器 {name}（{owner}）？实例与可写层将被清理",
    "已删除容器 {name}": "已删除容器 {name}",
    "名称": "名称",
    "所有者": "所有者",
    "镜像目录（sif 需先放到平台可访问的路径）": "镜像目录（sif 需先放到平台可访问的路径）",
    "已注册镜像 {name}": "已注册镜像 {name}",
    "显示名": "显示名",
    "可见性": "可见性",
    "镜像目录为空": "镜像目录为空",
    "公开": "公开",
    "私有": "私有",
    "镜像 {name} 已设为公开": "镜像 {name} 已设为公开",
    "镜像 {name} 已设为私有": "镜像 {name} 已设为私有",
    "确定删除镜像 {name}？（不影响已用该镜像创建的容器）": "确定删除镜像 {name}？（不影响已用该镜像创建的容器）",
    "已删除镜像 {name}": "已删除镜像 {name}",
    "+ 注册镜像": "+ 注册镜像",
    "名称（字母/数字/_./-）": "名称（字母/数字/_./-）",
    "注册": "注册",
    "资源": "资源",
    "停止": "停止",
    "启动": "启动",
    ...
    "镜像数": "镜像数",
    "24h 失败操作": "24h 失败操作",
    "执行器：{kind} · 数据库：{dialect}": "执行器：{kind} · 数据库：{dialect}",
    "配额层级（限制每用户可创建的容器/工作区数量与资源）": "配额层级（限制每用户可创建的容器/工作区数量与资源）",
    "+ 新建配额": "+ 新建配额",
    "确定删除配额 {name}？使用该配额的用户将失去配额限制依据": "确定删除配额 {name}？使用该配额的用户将失去配额限制依据",
    "已删除配额 {name}": "已删除配额 {name}",
    "配额已创建": "配额已创建",
    "配额已更新": "配额已更新",
    "快照/容器": "快照/容器",
    "编辑": "编辑",
    ...
    "磁盘": "磁盘",
    "搜索工作区名称": "搜索工作区名称",
    "确定删除工作区 {name}（{owner}）？存储目录将一并删除": "确定删除工作区 {name}（{owner}）？存储目录将一并删除",
    "已删除工作区 {name}": "已删除工作区 {name}",
    "没有工作区": "没有工作区",
    "大小": "大小",
    "文件数": "文件数",
    "来源容器": "来源容器",
    "个": "个",
    "LLM 访问授权（绑定平台用户与虚拟密钥预算）": "LLM 访问授权（绑定平台用户与虚拟密钥预算）",
    "用户 #id": "用户 #id",
    "预算": "预算",
    "授权": "授权",
    "已授权用户 #{id}（预算 {budget}/月）": "已授权用户 #{id}（预算 {budget}/月）",
    "确定撤销用户 #{id} 的 LLM 访问？其虚拟密钥将立即失效": "确定撤销用户 #{id} 的 LLM 访问？其虚拟密钥将立即失效",
    "已撤销用户 #{id} 的 LLM 访问": "已撤销用户 #{id} 的 LLM 访问",
    "没有 LLM 授权": "没有 LLM 授权",
    "周期": "周期",
    "模型": "模型",
    "全部": "全部",
    ...
    "生效中": "生效中",
    "撤销": "撤销",
    "按操作过滤（如 container.create）": "按操作过滤（如 container.create）",
    "没有日志": "没有日志",
    "时间": "时间",
    "条": "条",
  },
