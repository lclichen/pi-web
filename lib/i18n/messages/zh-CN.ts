...
export const zhCNLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip）。用户新建项目时可在向导中选择；已有项目可通过项目菜单「套用配置模板…」补装。": "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip）。用户新建项目时可在向导中选择；已有项目可通过项目菜单「套用配置模板…」补装。",
    "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip）。选择左侧模板查看详情，或点击左下角「上传模板」。": "配置模板是管理员维护的标准 .pi/ 配置 + labs/ 打包（zip）。选择左侧模板查看详情，或点击左下角「上传模板」。",
