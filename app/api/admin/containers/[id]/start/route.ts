...
// POST /api/admin/containers/[id]/start — admin lifecycle action on any
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await platformPost<unknown>("/api/v1/containers/" + id + "/start", identity.session.apiKey);
